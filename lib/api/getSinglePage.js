...
export default async function getSinglePage(
  ...
  try {
    const [versesRes, audioRes, surahListRes] = await Promise.all([
      fetch(
        `${QURAN_API_BASE_URL}/verses/by_page/${id}?per_page=300&language=${langCode}&words=true&word_fields=location,text_qpc_hafs,text_indopak,text_uthmani,code_v1,code_v2&translations=${identifier}`
      ),
      fetch(
