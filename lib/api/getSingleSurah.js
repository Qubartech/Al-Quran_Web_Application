...
import { getTranslatorName } from "@/lib/data/translationNames";
...
export default async function getSingleSurah(
  ...
  const isValidIdentifier = (str) => {
    ...
    return parts.length > 0 && parts.every((p) => !isNaN(Number(p)));
  };
  ...
  const idList = String(identifier)
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);

  try {
    const [versesRes, audioRes, chapterRes, segmentsRes, ...extraTransRes] = await Promise.all([
      fetch(
        `${QURAN_API_BASE_URL}/verses/by_chapter/${id}?per_page=300&language=${langCode}&words=true&word_fields=location,text_qpc_hafs,text_indopak,text_uthmani,text_uthmani_tajweed,code_v1,code_v2&translations=${idList[0]}`
      ),
      fetch(
      ...
      ...idList.slice(1).map((extraId) =>
        fetch(
          `${QURAN_API_BASE_URL}/verses/by_chapter/${id}?per_page=300&translations=${extraId}`
    ...
    const allTransResults = [versesData, ...extraTransRes.filter(Boolean)];
    ...
    const englishTransAyah = verses.map((v, i) => {
      const verseTransList = [];
      allTransResults.forEach((resData, rIdx) => {
        const verseMatch = resData?.verses?.[i];
        const tObj = verseMatch?.translations?.[0];
        if (tObj && tObj.text) {
          const resId = tObj.resource_id || idList[rIdx];
          verseTransList.push({
            text: tObj.text || "",
