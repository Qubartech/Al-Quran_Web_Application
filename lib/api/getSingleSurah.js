...
export default async function getSingleSurah(
  ...
  if (!identifier) throw new Error("Translation edition not found");
  ...
  try {
    const [versesRes, audioRes, chapterRes, segmentsRes] = await Promise.all([
      ...
      fetch(
        `https://api.quran.com/api/v4/recitations/${reciterId}/by_chapter/${id}`
      ),
      fetch(
        `https://api.quran.com/api/v4/chapters/${id}?language=en`
      ),
      fetch(
        `https://api.quran.com/api/v4/chapter_recitations/${reciterId}/${id}?segments=true`
      ),
    ]);

    if (!versesRes.ok || !audioRes.ok || !chapterRes.ok) {
      throw new Error("Failed to fetch Surah verses, audio, or chapter metadata");
    }
    ...
    return {
      data: [
        { ayahs: arabicAyah, englishName, arabicName, number: surahNumber, versesCount, revelationPlace, translatedName, audioUrl: segmentsData.audio_file?.audio_url || "" },
