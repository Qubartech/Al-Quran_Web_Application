...
export default async function getSingleSurah(
  ...
  try {
    const [versesRes, audioRes, chapterRes, segmentsRes, ...extraTransRes] = await Promise.all([
      fetch(
        `${QURAN_API_BASE_URL}/verses/by_chapter/${id}?per_page=300&language=${langCode}&words=true&word_fields=text_qpc_hafs,text_indopak,text_uthmani,code_v1,code_v2&translations=${idList[0]}`
      ),
      fetch(
