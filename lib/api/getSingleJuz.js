...
export default async function getSingleJuz(
  ...
  try {
    const [versesRes, audioRes, surahListRes] = await Promise.all([
      fetch(
        `${QURAN_API_BASE_URL}/verses/by_juz/${id}?per_page=600&language=${langCode}&words=true&word_fields=text_qpc_hafs,text_indopak,text_uthmani,code_v1,code_v2&translations=${identifier}`
      ),
      fetch(
