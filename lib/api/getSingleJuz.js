...
export default async function getSingleJuz(
  ...
  try {
    const [versesRes, audioRes, surahListRes] = await Promise.all([
      fetch(
        `${QURAN_API_BASE_URL}/verses/by_juz/${id}?per_page=600&language=${langCode}&words=true&word_fields=location,text_qpc_hafs,text_indopak,text_uthmani,code_v1,code_v2&translations=${identifier}`
      ),
      fetch(
        `${QURAN_API_BASE_URL}/recitations/${safeReciterId}/by_juz/${id}?per_page=600`
      ),
      getSurahList().catch(() => ({ data: [] }))
    ]);

    if (!versesRes.ok) {
      throw new Error(`Failed to fetch Juz ${id} verses`);
    }
    ...
    if (audioRes.ok) {
      ...
      try {
        const fallbackAudioRes = await fetch(
          `${QURAN_API_BASE_URL}/recitations/7/by_juz/${id}?per_page=600`
        );
    ...
    throw new Error(e.message || "Failed to load Juz data");
  }
}
